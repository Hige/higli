...
var path = {
    ...
    // 19, 21
    favicon: {
        master: "./app/master/favicon23.svg", // TODO: Path to your master picture
...
gulp.task("web-server", ["html", "scripts", "sass", "copyfonts"], function() {
    browserSync(configWebServer.main, configWebServer.callback);
});

gulp.task("scripts", function() {
    del.sync(path.clean.js);
    return runSequence(["javascripts", "iefix"], function(){
...
gulp.task("iefix", function() {
    return gulp.src(path.js.iefix.source)

        .on("error", handleError)
        .pipe(gulp.dest(path.js.iefix.destination))
        .pipe(size({
            title: "size of js"
        }))
        .pipe(browserSync.reload({stream: true}));
...
gulp.task("watch", function() {
    gulp.watch(path.js.watch, ["javascripts"]);
